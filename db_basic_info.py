...
class DatabaseBasicInfo:
    def __init__(self):
        ...
        self.col_acctinfo.create_index([('date', pymongo.DESCENDING), ('acctid', pymongo.ASCENDING)])
    ...
    def update_myacctsinfo(self):
        df_myacctsinfo = pd.read_excel(self.fpath_basicinfo, sheet_name='myacctsinfo',
                                       dtype={
                                           ...
                                           'BrokerAlias': str, 'DataSourceType': str, 'DataFilePath': str,
                                       })
        list_dicts_to_be_inserted = df_myacctsinfo.to_dict('records')
        for dict_to_be_inserted in list_dicts_to_be_inserted:
            dict_to_be_inserted['TradeDate'] = self.str_today
        self.col_acctinfo.delete_many({'TradeDate': self.str_today})
